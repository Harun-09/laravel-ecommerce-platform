...
class StripeGatewayService
{
    ...
    {
        return $this->createCheckoutSessionForOrders(
            $order,
            collect([$order]),
            collect([$payment])
        );
    ...
    public function createCheckoutSessionForOrders(Order $referenceOrder, Collection $orders, Collection $payments): array
    {
        ...
        if ($payments->isEmpty()) {
            return ['error' => 'No pending payment found for this checkout.'];
        }

        $primaryPayment = $payments->first();
        if (!$primaryPayment instanceof Payment) {
            return ['error' => 'Primary payment not found for this checkout.'];
        }

        $amountInMinor = (int) round(
        ...
        $successUrl = route('payment.stripe.success', $referenceOrder->order_number, true) . '?session_id={CHECKOUT_SESSION_ID}';
        $cancelUrl = route('payment.stripe.cancel', $referenceOrder->order_number, true);
        $customerEmail = $referenceOrder->shipping_email ?: $referenceOrder->user?->email;

        $lineItemName = $this->buildLineItemName($orders, $referenceOrder);
        $orderSummary = $this->buildOrderSummary($orders, $referenceOrder);
        ...
        $payload = [
            ...
            'client_reference_id' => (string) $primaryPayment->id,
            'metadata[order_id]' => (string) $referenceOrder->id,
            'metadata[order_number]' => (string) $referenceOrder->order_number,
            'metadata[payment_id]' => (string) $primaryPayment->id,
            'payment_intent_data[metadata][order_id]' => (string) $referenceOrder->id,
            'payment_intent_data[metadata][order_number]' => (string) $referenceOrder->order_number,
            'payment_intent_data[metadata][payment_id]' => (string) $primaryPayment->id,
            ...
            'line_items[0][price_data][product_data][description]' => $orderSummary,
            'line_items[0][quantity]' => 1,
        ];

        $this->appendBatchMetadata($payload, $orders, $payments, $referenceOrder);
        ...
        if (!empty($referenceOrder->user_id)) {
            $payload['metadata[user_id]'] = (string) $referenceOrder->user_id;
            $payload['payment_intent_data[metadata][user_id]'] = (string) $referenceOrder->user_id;
        }
    ...
    private function buildLineItemName(Collection $orders, Order $referenceOrder): string
    {
        if ($orders->count() > 1) {
            $count = $orders->count();
            return $this->displayName !== ''
                ? "{$this->displayName} Multi-vendor Checkout ({$count} orders)"
                : "Multi-vendor Checkout ({$count} orders)";
        }

        $order = $orders->first() ?? $referenceOrder;
        if (!$order instanceof Order) {
            return $this->displayName !== '' ? $this->displayName . ' Order Payment' : 'Order Payment';
        }
        ...
        $distinctItemCount = (int) $items->count();
        $primaryItemName = (string) ($items->first()?->product_name ?? '');
        $lineItemName = $primaryItemName !== ''
            ? Str::limit($primaryItemName, 60)
            : ($this->displayName !== '' ? $this->displayName . ' Order Payment' : 'Order Payment');

        if ($distinctItemCount > 1) {
            $lineItemName .= ' +' . ($distinctItemCount - 1) . ' more';
        }

        return $lineItemName;
    }

    private function buildOrderSummary(Collection $orders, Order $referenceOrder): string
    {
        if ($orders->count() > 1) {
            $firstOrderNumber = (string) ($orders->first()?->order_number ?? $referenceOrder->order_number);
            $additional = max(0, $orders->count() - 1);
            $summary = 'Orders ' . $firstOrderNumber;

            if ($additional > 0) {
                $summary .= ' +' . $additional . ' more';
            }

            $summary .= ' | ' . $orders->count() . ' vendor order' . ($orders->count() > 1 ? 's' : '');
            return $summary;
        }

        $order = $orders->first() ?? $referenceOrder;
        if (!$order instanceof Order) {
            return 'Order ' . $referenceOrder->order_number;
        }
    ...
    private function appendBatchMetadata(
        array &$payload,
        Collection $orders,
        Collection $payments,
        Order $referenceOrder
    ): void {
        $orderCount = max(1, (int) $orders->count());
        $paymentIds = $payments
            ->pluck('id')
            ->map(fn($id) => (string) $id)
            ->filter()
        ...
        $checkoutToken = trim((string) ($referenceOrder->checkout_token ?? ''));
        if ($checkoutToken === '' && $orders->isNotEmpty()) {
            $checkoutToken = trim((string) ($orders->first()?->checkout_token ?? ''));
        }

        if ($checkoutToken !== '') {
            $payload['metadata[checkout_token]'] = $checkoutToken;
            $payload['payment_intent_data[metadata][checkout_token]'] = $checkoutToken;
        }

        if ($orderCount > 1) {
            $payload['metadata[checkout_order_count]'] = (string) $orderCount;
            $payload['payment_intent_data[metadata][checkout_order_count]'] = (string) $orderCount;
        }

        // Stripe metadata value limit is 500 chars, keep room for growth.
        if ($paymentIds !== '' && strlen($paymentIds) <= 450) {
            $payload['metadata[payment_ids]'] = $paymentIds;
            $payload['payment_intent_data[metadata][payment_ids]'] = $paymentIds;
        }
    }
    ...
    private function isBrandingError(array $response): bool
    {
        ...
        return str_contains($error, 'branding_settings');
    }
    ...
    private function makeRequest(string $endpoint, string $method = 'GET', array $data = []): array
    {
        try {
            $request = Http::withBasicAuth($this->secretKey, '')
                ->withoutVerifying() // Disable SSL verification for servers without CA bundle
                ->asForm()
                ->acceptJson()
                ->timeout(30);
